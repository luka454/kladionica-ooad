    ...
    public class ClanKlubaDAO : IDaoCrud<ClanKluba>
    {
        ...
        public void delete(ClanKluba entity)
        {
            try
            {
                ...
                c = new MySqlCommand("delete from clanovi where id=" + id, DAL.Connection);
                c.ExecuteNonQuery();
            }
            catch (Exception ex)
            {
        ...
        public ClanKluba getById(int id)
        {
            try
            {
                c = new MySqlCommand("select * from clanovi where id=" + id, DAL.Connection);
        ...
        public List<ClanKluba> getAll()
        {
            try
            {
                c = new MySqlCommand("select * from clanovi", DAL.Connection);
                MySqlDataReader r = c.ExecuteReader();
        ...
        public List<ClanKluba> getByExample(string name, string value)
        {
            try
            {
                c = new MySqlCommand("select * from clanovi where ime=" + name + " and prezime=" + value, DAL.Connection);
                MySqlDataReader r = c.ExecuteReader();
                List<ClanKluba> clanovi = new List<ClanKluba>();
                while (r.Read())
                    clanovi.Add(new ClanKluba(r.GetString("ime"), r.GetString("prezime"), r.GetString("username"),
